...
A Python program that reads over a file and suggests spelling changes for the
document, giving recommendations using a dynamic programming implementation of
the classic minimum-edit-distance algorithm.
...
Efficiency:
- Minimum edit distance is only calculated for words that could potentially
  fall within a certain range. It is fairly uncommon for words to be shortened
  or extented in length incorrectly by more than a single character, as this
  usually leads to the user getting a feel for that the typed word is either
  too short or too long. We have therefore restricted the look-up of words to
  fall within this range.

- Minimum edit distance will only be calculated if the first couple of letters
  in the word are the same as the first couple of letters of the word from the
  dictionary, or when comparing last couple of letters to last couple of
  letters. Users typically start or finish a word correctly, as the starting
  and finishing sounds of a word are generally crucial to the word and
  therefore more emphasis is put on them when recoqnizing words. By doing this
  comparison and being selective in the minimum edit distances that we actually
  calculate the subset of words we are investigating fully is fairly small.
  The bound is fairly tight and an argument could be made for a larger bound,
  however, in this situation the author decided to make the bound of acceptance
  fairly tight due to the main goal of the assignment being to implement an
  algorithm to find the minimum edit distance.

- Capialization. Words that are not typically capitalized, but are at the
  beginning of a sentence and therefore are capitalized seem to confuse the
  dictionary, as the capitalized version of the word was not stored in the
  dictionary. To fix this problem we check whether the lowercase version of a
  word is in the dictionary instead. By doing so we do not need to search for
  replacements for the word 'We', for example - which saves us time.

Alterations to Levenshtein distance:
- Substitutions of letters that are close to each other on the keyboard are
  given a cost of 1, as opposed to 2. The decision to make this rule came about
  due to the fact that it is fairly common to miss the key that you are trying
  to press and accidently hit the key next to it. Which keys map to which keys
  are hardcoded as the dictionary 'surrounding'.
'''
